...
to partition circuits around where Hadamard gates appear. This creates partitions that
...
\begin{itemize}
  \item Create a partition
  \item For each qubit, sort any single non-Hadamard gates into the partition in the
    previous step. For CNOT-gates, check if the target qubit has terminated partitioning;
    if not, partition into the created partition.
  \item Terminate partitioning for a qubit when a Hadamard gate is encountered.
  \item Once all qubits have encountered a Hadamard gate, or they have reached the end of
    the circuit, save the partition.
  \item Redo the first step, starting the partition from after the Hadamard gate
    encountered for each qubit. Remove any qubits that have had all their gates partitioned.
  \item Terminate partitioning when all Hadamard gates have been processed.
\end{itemize}
...
A standard phase oracle drives the output of the Boolean function it implements on the
target qubit as demonstrated in~\figref{fig-single-target}. As a phase oracle implements a
single Boolean function by multiplying -1 on certain inputs, a phase oracle can be
constructed from a standard oracle as in~\figref{fig-single-target-phase}.

Observe that in general, barring the case that the Boolean function can be implemented
entirely using CNOT and X gates, this method creates at least three partitions. This
means that for most Boolean functions, the necessary T-depth may need to be doubled. 
...
any T-count, while the one with four variables cost 15. It is thus beneficial to find
an equivalent ESOP expression for a circuit that has as few cubes of over 4 variables as
