...
\item \textbf{(Minimize the number of $n \geq 4$ cubes in ESOP)} : The ESOP is optimized
  ...
  more than 4 literals. Our extensions to such methods is explained in
  \secref{Pro:Minimize}.
\item \textbf{(Map the $n \leq 3$ cubes to the 3 bit library)} : Takes all cubes of
  $n \leq 3$ and maps them to appropriate functions from the 3 bit library, inserting the
  corresponding subcircuit into the phase oracle.
