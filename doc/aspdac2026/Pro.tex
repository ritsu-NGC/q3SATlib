...
\item \textbf{(Generate 3bit phase polynomials)} : Precompute library of phase polynomials of Boolean functions of
  3 qubits or fewer using the Boolean Fourier transform.
\item \textbf{(Synthesize 3bit phase subcircuits)} : The phase polynomials are then sequenced using matroid
  partitioning from~\ref{bib-amy-matroid} and synthesized as subcircuits. The results are stored in hard disk,
...
\item \textbf{(Map the $n \leq 3$ cubes to the 3 qubit library)} : Takes all cubes of $n \leq 3$ and maps them to 
  appropriate functions from the 3 qubit library, inserting the corresponding subcircuit into the phase oracle.
...
discussion of its details is deffered to the cited paper. We modify it for our purposes by changing
the cost function \texttt{CountCubesInExactPseudoKro} EXORCISM-4 uses for its heuristic, which uses a plain sum
of cubes to assess the cost of the arrangement that its heuristic encounters. We instead introduce a cost function
...
we utilize the relative-phase Boolean decomposition method used in~\ref{bib-clarino-lut} to enable the
implementation of such $F$-controlled-NOT gates without any ancilla bits. We defer detailed discussion
of that method to the cited paper.
...
partitioning along the H-gates present in it. This deals with the circuits synthesized in~\ref{Pro:n4} as well as
the CNOT+T circuits the $n \leq 3$ processing
step produces. We defer more detailed discussion to the cited paper.
