...
{\it Quantum gates} describe transformations in qubits. The main ones that concern this work are
demonstrated in Fig.~\ref{fig-gates}. The $T/T^{\dagger}$, $S/S^{\dagger}$, and $Z$ gates will be
referred to as {\it phase gates}.

All of these gates besides $T/T^{\dagger}$ are known as {\it Clifford gates}. For
...
cost metric~\cite{amy-meet-in-middle}.
