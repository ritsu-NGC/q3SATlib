...
as the longest such chain of gates, known as the {\it T-depth}.
...
T-depth~\cite{amy-meet-in-middle,bib-amy-matroid,bib-selinger-tdepth}. In fact,
phase oracles of 3 or fewer variables can all be implemented exactly using this set
of gates~\cite{bib-amy-cnot}, and these circuits can be synthesized exactly using the
Boolean Fourier transform.

However, the vast majority of Boolean functions of 4 or more variables cannot be
implemented using only this gate set~\cite{bib-amy-rm}. The only fault-tolerant gate that can be
added to make this complete is the Hadamard (H) gate. The addition of this gate adds basis changes
that CNOT+T optimization methods cannot account for. Many existing methods to optimize for T-count
and T-depth try to account for the H-gate in the general case by introducing heuristic methods to
partition around it~\cite{amy-meet-in-middle,bib-amy-matroid,bib-amy-rm} or gadgetization to eliminate it
entirely~\cite{bib-heyfron}. The T-depth of methods which partition according to H-gates, such
...
Z-gate on an ancilla qubit. However, as we will see later, these types of constructions provide
an obstacle to optimizing their T-depth, owing to their use of Hadamard gates.
...
We find that our solution can reduce T-count and T-depth over known implementations
by an average of 36.5\% and over 65\% in some cases. We examine those cases and which
ones the method is best suited for.

The rest of this paper is structured as follows: First~\secref{Pre}
...
and CNOT+T circuits. Then~\secref{Mot} goes through some
motivational examples that inform the general idea of our proposal.
\secref{Pro} details the proposal itself, including how to
generate a library of up to 3-qubit phase oracles, and how to use
it to generate phase oracles of 4 or more qubits. In~\secref{Exp},
we test our results against Qiskit PhaseOracle and analyze our
findings. Finally,~\secref{Conc} concludes with our findings and
