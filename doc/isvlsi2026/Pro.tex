...
To synthesize cubes of $n \geq 4$ variables, we utilize the construction in ~\secref{Pre:oracle}. We add an
ancilla bit and use the relative-phase MCT synthesis method from~\cite{bib-rtof-maslov} to enable the
implementation of such $F$-controlled-NOT gates with a reduced number of T-gates. We defer detailed
discussion of that method to the cited paper. This resulting phase oracle has T-count $2^{n}$, and
T-depth $2n+1$. Note that this means that larger cubes can dominate the T-depth of a given circuit, so it is
imperative that the minimum of them be found for
