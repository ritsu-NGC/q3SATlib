...
with as little cubes of over four literals as possible. This allows us to reduce the number
...
\item \textbf{(Synthesize logic network and QC from $n \geq 4$ cubes)} : Generates a logic network from all cubes
  of $n \geq 4$ as a logic
  network with the values of each of the cubes as separate outputs, and synthesizes the quantum circuit as a
  network of Toffoli gates and Controlled-Z gates, as described in Sec.~\ref{Pro:n4}.
...
which one we pick in our section on the implemention in~\secref{Exp}.
